...
export const getBoothStatus = async (): Promise<Booth> => {
  ...
  return transtromGetBoothStatus(response);
};
...
export const postBoothOperation = async (status: "paused" | "resume") => {
  try {
    await postNoResponse<null>(`/api/v1/manager/booth/status/${status}`, null);
  } catch (error) {
    // TODO: -분기처리 세세하게
    throw error;
  }
